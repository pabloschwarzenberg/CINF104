from keras.layers import Attention, Flatten, Input, Dense, Concatenate
...
head=[]
head.append(Attention()([q,v,k]))
head.append(Attention()([q,v,k]))
head.append(Attention()([q,v,k]))
head.append(Attention()([q,v,k]))
concat=Concatenate()(head)
...
tag="matt_"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
